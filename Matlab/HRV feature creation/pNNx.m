...
function pNNx(pat,saving,win,loadfolder,savefolder)
 Neonate=pat;
        ...
        if exist('consence_RR_windowed', 'var')==1
              for k=1:length(consence_RR_windowed)            
                  if isempty(consence_RR_windowed{1,k})==0 & sum(isnan(consence_RR_windowed{1,k}(2,:)))==0
                      pNN50{1,k}=(sum(abs(diff(consence_RR_windowed{1,k}(2,:)))>=50)/length(consence_RR_windowed{1,k}(2,:)))*100;
                      pNN50{2,k}=merged_annotation{1,k};
                      pnn50{3,k}=annotations;
                      
                      pNN30{1,k}=(sum(abs(diff(consence_RR_windowed{1,k}(2,:)))>=30)/length(consence_RR_windowed{1,k}(2,:)))*100;
                      pNN30{2,k}=merged_annotation{1,k};
                      pnn30{3,k}=annotations; 
                      
                      pNN20{1,k}=(sum(abs(diff(consence_RR_windowed{1,k}(2,:)))>=20)/length(consence_RR_windowed{1,k}(2,:)))*100;
                      pNN20{2,k}=merged_annotation{1,k};
                      pnn20{3,k}=annotations;  
                      
                      pNN10{1,k}=(sum(abs(diff(consence_RR_windowed{1,k}(2,:)))>=10)/length(consence_RR_windowed{1,k}(2,:)))*100;
                      pNN10{2,k}=merged_annotation{1,k};
                      pnn10{3,k}=annotations;      
                  else
                      pNN50{1,k}=[];
                      pNN30{1,k}=[];
                      pNN20{1,k}=[];
...
%%%%%%%%%%%% SAVING            
    if saving                     %saving R peaks positions in mat file                 
       Saving(pNN50,savefolder,Neonate,win(1,j)) 
       Saving(pNN30,savefolder,Neonate,win(1,j)) 
       Saving(pNN20,savefolder,Neonate,win(1,j)) 
       Saving(pNN10,savefolder,Neonate,win(1,j))        
    end% end if saving  
