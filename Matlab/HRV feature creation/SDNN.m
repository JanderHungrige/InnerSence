% SDNN
function SDNN(consence_RR_windowed,pat,saving,win,loadfolder,savefolder)
Neonate=pat;
     ...
     if exist('consence_RR_windowed','var')==1   
         for k=1:length(consence_RR_windowed)
            if isempty(consence_RR_windowed{1,k})==0 & sum(isnan(consence_RR_windowed{1,k}(2,:)))==0
                SDNN{1,k}=nanstd(consence_RR_windowed{1,k}(2,:));
                SDNN{2,k}=merged_annotation{1,k};
                SDNN{3,k}=annotations;
            else
                SDNN{1,k}=[];
            end
         end
...
%%%%%%%%%%%% SAVING            
    if saving                     %saving R peaks positions in mat file                 
       Saving(SDNN,savefolder,Neonate,win(1,j)) 
    end% end if saving   
