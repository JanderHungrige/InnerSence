...
explus=squ1 + (oddi1.*eveni1);
explus = [ex(1); ex; ex(length(sig)-2)]; %make it the same length
%% plots

if gr
figure,ax(1)=subplot(211);plot((sig/max(sig))-mean(sig/max(sig)),'b'),
hold on,
plot((ey/max(ey))-mean(ey/max(ey)),'Linewidth',1,'color','r'),
axis tight;
title '-';
...
hold on,
plot((ex/max(ex))-mean(ex/max(ex)),'Linewidth',1,'color','g'),
...
figure,ax(1)=subplot(211);plot((sig/max(sig))-mean(sig/max(sig)),'b'),
hold on,
plot((eyplus/max(eyplus))-mean(eyplus/max(eyplus)),'Linewidth',1,'color','r'),
axis tight;
title '+';
hleg1=legend('Original Signal','Energy Operator');
set(hleg1,'Location','NorthWest')
ax(2)=subplot(212);plot((sig/max(sig))-mean(sig/max(sig)),'b'),
hold on,
plot((explus/max(explus))-mean(explus/max(explus)),'Linewidth',1,'color','g'),
hleg2=legend('Original Signal','Teager Energy');
set(hleg2,'Location','NorthWest')
axis tight,
zoom on;
