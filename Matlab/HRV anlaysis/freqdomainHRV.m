...
function freqdomainHRV (pat,plotting,win,saving)
Neonate=pat;
...
%%%%% AS QS
          if exist('pxxAS','var')
           for i=1:length(LF_AS)
                    LFnorm_AS(i)=((cell2mat(LF_AS(1,i)))./(totpowAS(1,i)-cell2mat(VLF_AS(1,i))))*100; % before LFnorm_AS=((cell2mat(LF_AS))./totpowAS)*100;
%                     LFnorm_AS(LFnorm_AS==0)=nan; %all zeroes to nan to avoid confusion between AS and QS
           end
          end
          
          if exist('pxxQS','var')
               for i=1:length(LF_QS)
                    LFnorm_QS(i)=((cell2mat(LF_QS(1,i)))./(totpowQS(1,i)-cell2mat(VLF_QS(1,i))))*100;
...
%%%%%%%%% AS QS
              if exist('pxxAS','var')
                  for i=1:length(HF_AS)
                    HFnorm_AS(i)=((cell2mat(HF_AS(1,i)))./(totpowAS(1,i)-cell2mat(VLF_AS(1,i))))*100;     
%                     HFnorm_AS(HFnorm_AS==0)=nan; %all zeroes to nan to avoid confusion between AS and QS
                  end
              end
              
              if exist('pxxQS','var')
                  for i=1:length(HF_QS)
                    HFnorm_QS(i)=((cell2mat(HF_QS(1,i)))./(totpowQS(1,i)-cell2mat(VLF_QS(1,i))))*100;     
