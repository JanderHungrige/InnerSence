...
% THIS VERSION IS OPTIMIZED FOR THE IN-HOUSE ABDOMINAL DATABASE !!
    
    if nargin < 6; g.BLOCK_SIZE  = 1024; else g.BLOCK_SIZE = blockSize; end;
    ...
    while sampleThreshold + g.BLOCK_SIZE < g.NR_OF_SAMPLES
        ...
        minThresholdValue = (1/8)*(mean(waveletOutput)*4) + (7/8)*minThresholdValue;
        ...
        else [ratio] = calculateSNR(abs(waveletOutput), peakPosition, Fs); end
        ...
        if ~isnan(peakPosition); out.peakPositionArray(end+1) = startOfSegment + peakPosition; end
...
%         plotSignals(inputData(1,:),out.peakPositionArray(1,:),out.waveletOutputArray(1,:),out.thresHoldValueArray(1,:),nan,out.startOfSegmentArray(1,:),out.endOfSegmentArray(1,:),Fs, out.waveletArray);
    ...
    plot(inputData(1,:))
    hold on
    plot(out.peakPositionArray(1,:),inputData(1,out.peakPositionArray(1,:)),'x r')
    hold off
